...
if(!empty($_POST['url'])&& $_POST['url']!= $config_default_ajout_site_url)
{
    if(!empty($_POST['nom_site'])&& $_POST['nom_site'] != $config_default_ajout_nom_site)
    {
        ...
        $req['ajout_dossier'] ="UPDATE dossiers SET url = :url, nom_site = :nom_site WHERE id = :id_dossier" ; 
        ...
        header('Location: modifier_site.php?e='.urlencode('Le site a bien était modifié').'&id_dossier='.$_POST['id_dossier']) ; 
    }
